...
import cn.encore.framecommon.R;

/**
 * Created by：Encore
 * Created Time：16/7/1 16:12
 */
public class SettingUtils {
    ...
    public static boolean getOnlyWifiLoadImg(Context ctx) {
        return SpUtils.getInstance(ctx).getBoolValue(ONLY_WIFI_LOAD_IMG);
    }

    public static void setOnlyWifiLoadImg(Context ctx, boolean isOn) {
        SpUtils.getInstance(ctx).setValue(ONLY_WIFI_LOAD_IMG, isOn);
    }
    ...
    public static String getCurrentTheme(Context ctx) {
        if(TextUtils.isEmpty(SpUtils.getInstance(ctx).getStringValue(CURRENT_THEME))){
            return "TRSDayTheme_DEFAULT";
        }
        return SpUtils.getInstance(ctx).getStringValue(CURRENT_THEME);
    }

    public static void setCurrentTheme(Context ctx, String themeName) {
        SpUtils.getInstance(ctx).setValue(CURRENT_THEME, themeName);
    }
